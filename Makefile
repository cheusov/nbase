...
#PROJECTS = bdes							\
  ...
  rlogin rsh rup ruptime rusers rwall rwho script \
  stat \
...
PROJECTS  =	apply nawk/bin banner basename cat chmod cleantags \
  ...
  lam ln ls m4 mkdep mkdir mkfifo mktemp mv nl nohup paste patch pax printenv \
  printf pwd rev rm rmdir rs sdiff sed seq shar shlock shuffle \
