...
#PROJECTS = bdes							\
  ...
  seq shar shlock shuffle soelim sort split stat tabs tee test	\
...
PROJECTS  =	apply nawk/bin banner basename cat chmod cleantags \
  ...
  domainname echo \
  ed env error \
  expand expr false head hostname kill \
  ln ls mkdir mv pax pwd rm rmdir sleep stty tail testcmd true wc yes
